...
describe Oakdex::Breeding do
  let(:pokemon1) { Oakdex::Pokemon.create('Ditto', level: 10) }
  let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male') }

  describe '.compatible?' do
    it { expect(described_class).to be_compatible(pokemon1, pokemon2) }

    context 'both dittos' do
      let(:pokemon2) { Oakdex::Pokemon.create('Ditto', level: 20) }
      it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
    end

    context 'neutral' do
      let(:pokemon2) { Oakdex::Pokemon.create('Rotom', level: 20) }
      it { expect(described_class).to be_compatible(pokemon1, pokemon2) }

      context 'both neutral and same species but not ditto' do
        let(:pokemon1) { Oakdex::Pokemon.create('Rotom', level: 10) }
        it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
      end

      context 'one neutral and same egg group but not ditto' do
        let(:pokemon1) { Oakdex::Pokemon.create('Chandelure', level: 10, gender: 'female') }
        it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
      end
    end

    context 'same egg group' do
      let(:pokemon1) { Oakdex::Pokemon.create('Pachirisu', level: 10, gender: 'female') }
      it { expect(described_class).to be_compatible(pokemon1, pokemon2) }

      context 'both female' do
        let(:pokemon1) { Oakdex::Pokemon.create('Pachirisu', level: 10, gender: 'female') }
        let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'female') }
        it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
      end

      context 'both male' do
        let(:pokemon1) { Oakdex::Pokemon.create('Pachirisu', level: 10, gender: 'male') }
        let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male') }
        it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
      end
    end
    ...
    context 'different egg group' do
      let(:pokemon1) { Oakdex::Pokemon.create('Bulbasaur', level: 10, gender: 'female') }
      it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
    end

    context 'same egg group but undiscovered' do
      let(:pokemon1) { Oakdex::Pokemon.create('Pichu', level: 10, gender: 'female') }
      let(:pokemon2) { Oakdex::Pokemon.create('Pichu', level: 10, gender: 'male') }
      it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
    end

    context 'both Pikachus' do
      let(:pokemon1) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'female') }
      let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male') }
      it { expect(described_class).to be_compatible(pokemon1, pokemon2) }
      ...
      context 'both male' do
        let(:pokemon1) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male') }
        let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male') }
        it { expect(described_class).not_to be_compatible(pokemon1, pokemon2) }
      end
    end
  ...
  describe '.breed' do
    ...
    context 'parent with egg moves' do
      ...
      let(:pokemon1) { Oakdex::Pokemon.create('Ditto', level: 10) }
      let(:pokemon2) { Oakdex::Pokemon.create('Pikachu', level: 20, gender: 'male', moves: parent_moves) }
